...
def CheckExtension(out_file):
    ...
    elif isinstance(out_file, pathlib.PosixPath) or isinstance(
        out_file, pathlib.WindowsPath
    ):
...
def CheckColumnsNotList(df):
    ...
    if any(type_list):
        ...
        raise TypeError(
            "No column can be of type list - check the following columns: "
            + ", ".join(type_list_columns)
        )
...
def CheckKeys(df, keys):
    ...
    if keys_with_missing.any():
        ...
        raise ValueError(
            f"The following keys are missing in some rows: {missings_string}."
        )

    type_list = any(
        [any(df[keycol].apply(lambda x: type(x) == list)) for keycol in keys]
    )
    if type_list:
...
def GetSummaryStats(df):
    ...
    summary_stats = pd.DataFrame({"type": var_types}).merge(
        var_stats, how="left", left_index=True, right_index=True
    )
    summary_stats = summary_stats.round(4)

    comma_sep_cols = [
        col for col in summary_stats.columns if col not in ["variable_name", "type"]
    ]
