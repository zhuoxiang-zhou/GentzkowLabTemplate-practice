...
Arguments:
    filename  : Path to save the data (supported formats: .dta, .csv)
    key       : Key variable(s) that uniquely identify rows

Options:
    if        : Condition to filter data before saving
    outsheet  : Use the `outsheet` command for exporting data
    log_replace : Overwrite the existing log file
    log(str)  : Path to log summary statistics (default: ../data_file_manifest.log)
    missok    : Continue even if there are missing values
    nopreserve: Do not preserve the dataset in memory after saving
* ============================================================================= */
