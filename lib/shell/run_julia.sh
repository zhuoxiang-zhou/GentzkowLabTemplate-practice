...
run_julia () {
    ...
    # set julia command if unset
    if [ -z "$juliaCmd" ]; then
        echo -e "\nNo Julia command set. Using default: julia"
        juliaCmd="julia"
    fi

    # check if the command exists before running, log error if does not
    if ! command -v ${juliaCmd} &> /dev/null; then
        error_time=$(date '+%Y-%m-%d %H:%M:%S')
        echo -e "\n\033[0;31mProgram error\033[0m at ${error_time}: ${juliaCmd} not found. Make sure command line usage is properly set up." 
        echo "Program Error at ${error_time}: ${juliaCmd} not found." >> "${logfile}"
    ...
    echo -e "\nScript ${program} in ${juliaCmd} started at ${start_time}" | tee -a "${logfile}"

    # run command and write stdout and stderr directly to the log file
    ${juliaCmd} "${program}" >> "${logfile}" 2>&1
